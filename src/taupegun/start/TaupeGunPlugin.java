...
public class TaupeGunPlugin extends JavaPlugin{
	 ...
	 */
	private String scoreboardTitle = "Taupe Gun INSA";
				...
				if (!context.hasStarted())
				{
					...
					Bukkit.getScheduler().runTaskLater(this, new Runnable()
					{
						public void run()
						{
							...
							TaupeGunPlugin.getPlugin().getServer().broadcastMessage(ChatColor.GOLD+"You are no longer invincible");
						}
	...
	public void MatchInfo()
	{
		...
		if (!context.isMolesActivated())
		{
			...
			ob.getScore(ChatColor.RED + "Moles" + ChatColor.GRAY + " in :").setScore(-7);
		...
		if(seconds == -1)
		{
			...
			if (minutes == -1)
			{
				...
				if (plugin.getConfig().getInt("map.startResizeEpisodeNumber") == plugin.getContext().getEpisode())
				{
					...
					txt = ChatColor.DARK_RED + "The world is reducing...";
			...
			if (!plugin.getContext().isMolesActivated())
			{
				...
				if(minutes == -1)
				{
					...
					for (Player player : plugin.getContext().getMoles())
					{
						...
						player.sendMessage(ChatColor.GOLD + "You are a mole ! Now your teammates are the other moles ! Fool your old teammates !");
						player.sendMessage(ChatColor.GOLD + "Use '/reveal' to reveal you as a mole at any time !");
						player.sendMessage(ChatColor.GOLD + "Use '/t (message)' to talk to other moles.");
